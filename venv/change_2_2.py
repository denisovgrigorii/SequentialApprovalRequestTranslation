...
def main_f():
    ...
    #сериализация json файла
        sequential_approval_request = json.load(json_file)
    raw_in_column = 2
    ...
    for i in sequential_approval_request.keys():
        ...
        if sequential_approval_request[name_ir_list[number_name_ir_list]]['managerStage']['isEnabled'] == True and len(row_role) !=0:
...
def find_value(name_value_for_pars: list, url_translation_ru: str) -> list:
    dict_value = {}
    if len(name_value_for_pars) > 0:
        with open(url_translation_ru, 'r') as dict_file:
            ...
            dict_file.close()
        with open('/opt/ankey/ankey/ui/default/ng/public/i18n/translation_ru.properties', 'r') as dict_file:
            for row in dict_file:
                if len(dict_value) < len(name_value_for_pars):
                    find_value = [x for x in name_value_for_pars if x in row]
                    if find_value:
